...
@app.post("/inference/arcane_style", response_model=RestoreResp)
@limiter.limit("20/minute")
async def arcane_style_multipart(
    request: Request,
    authorized=Depends(require_api_key),
    image: UploadFile = File(..., description="Ảnh gốc (jpg/png/webp)"),
    prompt: str = Form(
        "a character in Arcane style, vibrant colors",
        description="Chỉ dẫn phong cách (mặc định: Arcane style)",
    ),
    strength: float = Form(0.75, description="Độ thay đổi phong cách [0.0-1.0]"),
    num_inference_steps: int = Form(20, description="Số bước inference [10-50]"),
    guidance_scale: float = Form(7.5, description="Độ ảnh hưởng của prompt [1.0-20.0]"),
):
    """
    Chuyển đổi ảnh sang phong cách anime Arcane sử dụng nitrosocke/Arcane-Diffusion.
    - Nhận ảnh (multipart) và prompt (form data) từ client.
    - Cache kết quả (ảnh PNG) lên Cloudflare R2.
    """
    # 1. Validate tham số
    if not (0.0 <= strength <= 1.0):
        raise HTTPException(422, "strength must be in [0.0, 1.0]")
    if not (10 <= num_inference_steps <= 50):
        raise HTTPException(422, "num_inference_steps must be in [10, 50]")
    if not (1.0 <= guidance_scale <= 20.0):
        raise HTTPException(422, "guidance_scale must be in [1.0, 20.0]")
    if not prompt:
        raise HTTPException(422, "Prompt cannot be empty")
    ...
    # 3. Gọi HF endpoint
    try:
        out_bytes, meta = await call_arcane_style_from_bytes(
            img_bytes=img_bytes,
            prompt=prompt,
            strength=strength,
            num_inference_steps=num_inference_steps,
    ...
    meta.update(
        {
            "r2_key": key,
            "cache_hit": hit,
            "model": meta.get("model", "nitrosocke/Arcane-Diffusion"),
        }
    )
